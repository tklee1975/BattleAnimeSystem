...
public class CFX_Demo_New : MonoBehaviour
{
	...
	void Awake()
	{
		...
		particleExampleList.Sort( delegate(GameObject o1, GameObject o2) { 
	...
	{
		if(ParticleExamples == null) {
			Debug.Log("ParticleExample is null");
			return;
		}

		GameObject obj = ParticleExamples[exampleIndex];
		if(obj == null) {
			Debug.Log("ParticleExample-" + exampleIndex + " is null");
			return;
		}
		Debug.Log("UpdateUI: for object=" + obj);
		if(EffectLabel == null){ 
			return;
		}
		EffectLabel.text = obj.name;
