...
public partial class SimpleAnimationPlayable : PlayableBehaviour
{
    ...
    public bool SetClip(AnimationClip clip, string name)
    {
        StateInfo state = m_States.FindState(name);
        if (state == null)
        {
            Debug.Log("SimpleAnimation: state is null. name=" + name + " try to AddClip");
            return AddClip(clip, name);
            //return false;
        }

        var clipPlayable = AnimationClipPlayable.Create(graph, clip);
        if (!clip.isLooping || state.wrapMode == WrapMode.Once)
        {
            clipPlayable.SetDuration(clip.length);
        }
        state.SetPlayable(clipPlayable);
        state.Pause();
